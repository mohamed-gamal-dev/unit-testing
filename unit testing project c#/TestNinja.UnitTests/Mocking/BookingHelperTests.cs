...
{
    // we transferred the method name to the class name to make the method name shorter
    [TestFixture]
    internal class BookingHelper_OverlappingBookingsExistTests
    {
        ...
        [Test]
        public void BookingIsOverlapButNewBookingIsCancelled_ReturnEmptyString()
        {
            var result = BookingHelper.OverlappingBookingsExist(new Booking
            {
                Id = 1,
                // no matter arrival and departure here you can set them with any value because it is already cancelled
                ArrivalDate = After(_existingBooking.DepartureDate),
                DepartureDate = After(_existingBooking.DepartureDate, days: 2),
                Status= "Cancelled"
            }, _repository.Object);
